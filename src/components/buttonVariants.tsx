...
import { Slot } from '@radix-ui/react-slot' // Import Slot from Radix for conditionally rendering child components
import { cva, type VariantProps } from 'class-variance-authority' // Import cva to define dynamic variants for styling
import ReactLoading from 'react-loading' // Import loading spinner component for loading state
import { cn } from '@/utils/cn' // Utility function to merge class names

// Define button variants with the cva function to apply different styles based on props
const buttonVariants = cva(
  'inline-flex items-center whitespace-nowrap overflow-ellipsis justify-center rounded-md text-sm font-medium transition-all duration-300 focus-visible:outline-none disabled:pointer-events-none gap-2', // Default base styles
  {
    variants: {
      // Define color variants for different button types (default, danger, primary, etc.)
      colorVariant: {
        nothing: [],
        default: [
          'bg-state-default',
    ...
    defaultVariants: {
      ...
      colorVariant: 'default', // Default color variant
      layout: 'default', // Default layout
    },
  },
)

// Define props for the Button component, extending from button attributes and buttonVariants props
export interface ButtonProps
  extends React.ButtonHTMLAttributes<HTMLButtonElement>,
    VariantProps<typeof buttonVariants> {
  asChild?: boolean // Option to render a child component instead of a <button>
  ...
  (
    {
      ...
      layout,
      asChild = false,
      isLoading, // When true, show a loading spinner
      ...props
    },
    ref,
  ) => {
    const Comp = asChild ? Slot : 'button' // If asChild is true, render Slot (child component) else render <button>

    return (
      <Comp
        className={cn(
          buttonVariants({ variant, colorVariant, layout, size, className }), // Apply dynamic class names based on props
        )}
        ref={ref}
        {...props} // Spread other props like onClick, disabled, etc.
      >
        {isLoading && ( // If loading, show a loading spinner
          <ReactLoading type={'spin'} color="grey" height={24} width={24} />
        )}
        {props.children} {/* Render button children */}
      </Comp>
    )
  },
)
Button.displayName = 'Button' // For easier identification in debugging

// Export Button and buttonVariants for use in other components
